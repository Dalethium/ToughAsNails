	...
	private byte[] transformEntityRenderer(byte[] bytes, boolean obfuscatedClass) {
		...
		for (MethodNode methodNode : classNode.methods) {
			if (ASMHelper.methodEquals(methodNode, RENDER_RAIN_SNOW_NAMES, "(F)V")) {
				MethodInsnNode targetMethodInsnNode = ASMHelper.getUniqueMethodInsnNode(methodNode, Opcodes.INVOKEVIRTUAL,
						ObfHelper.unmapType(obfuscatedClass, "net/minecraft/world/biome/Biome"), GET_FLOAT_TEMPERATURE_NAMES,
				...
				targetMethodInsnNode.desc = ObfHelper.createMethodDescriptor(obfuscatedClass, "F", "net/minecraft/world/biome/Biome",
		...
		if (successfulTransformations.size() != 2) throw new RuntimeException(
				"An error occurred transforming EntityRenderer. Applied transformations: " + successfulTransformations.toString());
