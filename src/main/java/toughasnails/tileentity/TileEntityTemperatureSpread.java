...
public class TileEntityTemperatureSpread extends TileEntity implements ITickable, ITemperatureRegulator {
	...
	public void update() {
		...
		if (++updateTicks % 20 == 0) {
			...
			if (this.maxSpreadBox == null) {
				this.maxSpreadBox = new AxisAlignedBB(this.pos.getX() - MAX_SPREAD_DISTANCE, this.pos.getY() - MAX_SPREAD_DISTANCE,
						this.pos.getZ() - MAX_SPREAD_DISTANCE, this.pos.getX() + MAX_SPREAD_DISTANCE, this.pos.getY() + MAX_SPREAD_DISTANCE,
			...
			for (EntityPlayer player : world.getEntitiesWithinAABB(EntityPlayer.class, this.maxSpreadBox)) {
				...
				outer: for (int i = MAX_SPREAD_DISTANCE - distance; i >= 0; i--) {
					for (BlockPos pos : this.filledPositions[i]) {
						...
						if (player.getEntityBoundingBox().intersects(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1D, pos.getY() + 1D,
								pos.getZ() + 1D)) {
							collided = true;
							break outer;
						}
				...
				if (!world.isRemote && spawnedEntities.isEmpty() && !this.filledPositions[MAX_SPREAD_DISTANCE].isEmpty()) {
					for (int strength = 0; strength <= MAX_SPREAD_DISTANCE; strength++) {
						for (BlockPos pos : this.filledPositions[strength]) {
							final AxisAlignedBB boundingBox = new AxisAlignedBB(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1.0D,
							...
							Predicate<EntitySmallFireball> predicate = new Predicate<EntitySmallFireball>() {
								...
								public boolean apply(EntitySmallFireball input) {
									// Check intersections with this entity a little bit over 1x1x1, because it seemed the outer layer of
									// fireballs wasn't
									// included otherwise
									BlockPos pos = input.getPosition();
									return boundingBox.intersects(pos.getX() - 0.1D, pos.getY() - 0.1D, pos.getZ() - 0.1D, pos.getX() + 1.1D,
											pos.getY() + 1.1D, pos.getZ() + 1.1D);
								}
							};
	...
	public void fill() {
		...
		for (EnumFacing facing : EnumFacing.values()) {
			...
			if (this.canFill(offsetPos)) this.filledPositions[MAX_SPREAD_DISTANCE].add(offsetPos);
		}
	...
	private boolean canFill(BlockPos pos) {
		...
		return !this.getWorld().isBlockFullCube(pos)
				&& (!this.getWorld().canSeeSky(pos) || this.getWorld().getBlockState(pos).getBlock() instanceof IDecayableCrop);
	}
