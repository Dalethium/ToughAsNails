...
public class Temperature {
	...
	public int getRangeIndex(boolean reverseEnd) {
		if (rangeIndex == -1) return TemperatureScale.getRangeIndex(getRawValue(), reverseEnd);

		return rangeIndex;
	}

	public float getRangeDelta(boolean reverseEnd) {
		if (rangeDelta == -1F) rangeDelta = TemperatureScale.getRangeDelta(getRawValue(), reverseEnd);
