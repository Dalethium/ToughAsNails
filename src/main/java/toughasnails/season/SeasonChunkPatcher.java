...
public class SeasonChunkPatcher {
	...
	public void notifyLoadedAndPopulated(World world, ChunkPos chunkPos) {
		...
		if (chunkData != null) {
			...
			for (int i = 0; i < ChunkKey.NEIGHBORS.length; i++) {
				if (!chunkData.isNeighborToBeNotified(i)) continue;
	...
	public void enqueueChunkOnce(Chunk chunk) {
		...
		if (pendingChunksMask.contains(key)) return;
	...
	public void onChunkUnload(Chunk chunk) {
		...
		if (data != null) internRemoveFromQueue(data);
	}
	...
	public void onServerWorldUnload(World world) {
		...
		for (ActiveChunkData ac : updatedChunksQueue) {
			if (ac.getWorld() == world) ac.detach();
			else chunks.add(ac);
		}
	...
	public void onServerTick() {
		...
		for (PendingChunkEntry entry : chunksInProcess) {
			if (numProcessed >= numPatcherPerTick) break;
			numProcessed++;
			...
			if (unavailableChunkMask != 0) {
				...
				for (int i = 0; i < ChunkKey.NEIGHBORS.length; i++) {
					int bit = 0x1 << i;
					if ((unavailableChunkMask & bit) == 0) continue;
	...
	private void executePatchCommand(int command, long snowyTrackTicks, long rainingTrackTicks, Chunk chunk, Season season) {
		if (command != 0) {
			...
			if (command == 2) {
				...
				if (dur > snowyTrackTicks) dur = snowyTrackTicks;
				threshold = evalProbUpdateTick((int) dur);
			} else if (command == 1 || command == 3) threshold = evalProbUpdateTick((int) snowyTrackTicks);
	...
	private void patchChunkTerrain(ChunkData chunkData) {
		...
		if (bFastForward) {
			if (bWasSnowy) executePatchCommand(4, 0, chunk, season);
			else executePatchCommand(5, 0, chunk, season);
		}
