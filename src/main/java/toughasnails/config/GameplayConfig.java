...
public class GameplayConfig extends ConfigHandler {
	...
	protected void loadConfiguration() {
		try {
			addSyncedValue(GameplayOption.ENABLE_LOWERED_STARTING_HEALTH, true, SURVIVAL_SETTINGS,
			...
			easyStartingHearts =
					config.getInt("Easy starting hearts", STARTING_HEALTH, 7, 1, 10, "Amount of hearts players will start with on easy");
			normalStartingHearts =
					config.getInt("Normal starting hearts", STARTING_HEALTH, 5, 1, 10, "Amount of hearts players will start with on normal");
			hardStartingHearts =
					config.getInt("Hard starting hearts", STARTING_HEALTH, 3, 1, 10, "Amount of hearts players will start with on hard");
