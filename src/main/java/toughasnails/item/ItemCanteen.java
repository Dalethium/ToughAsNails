...
public class ItemCanteen extends Item {
	...
	public int getUses(NBTTagCompound comp) {
		if (comp == null) comp = new NBTTagCompound();
		int uses = comp.getInteger("uses");
		if (uses == 0) uses = -1;
		if ((uses > 0 && uses < 4) || uses == -1) return uses;
		return -1;
	}
	...
	private NBTTagCompound doLore(NBTTagCompound comp, ItemStack stack) {
		return setLore(comp, new String[] { "Refills: " + getRefills(comp) + " / " + REFILLS,
				"Uses Left: " + (getUses(comp) > 0 ? getUses(comp) : "0") + " / 3" }); // TODO % value at the end
	}
	...
	*/
	public NBTTagCompound doRefill(NBTTagCompound comp, ItemStack stack, EntityPlayer player) {
