...
public class ItemBottleOfGas extends Item {
	...
	@Override
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing side, float hitX, float hitY,
			float hitZ) {
		...
		if (!playerIn.canPlayerEdit(pos, side, stack)) {
			...
			if (this.block.canPlaceBlockOnSide(worldIn, pos, side)) {
				IBlockState iblockstate1 =
						this.block.getStateForPlacement(worldIn, pos, side, hitX, hitY, hitZ, this.getMetadata(stack), playerIn);
