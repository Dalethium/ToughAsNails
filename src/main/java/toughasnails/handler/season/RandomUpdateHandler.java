				...
				for (Iterator<Chunk> iterator = world.getPersistentChunkIterable(world.getPlayerChunkMap().getChunkIterator());
					...
					if (world.rand.nextInt(rand) == 0) {
						...
						if (world.getBlockState(groundPos).getBlock() == Blocks.ICE
								&& !SeasonHelper.canSnowAtTempInSeason(season, world.getBiome(groundPos).getFloatTemperature(groundPos))) {
							((BlockIce) Blocks.ICE).turnIntoWater(world, groundPos);
						}

						if (world.getBlockState(topPos).getBlock() == Blocks.SNOW_LAYER
								&& !SeasonHelper.canSnowAtTempInSeason(season, world.getBiome(topPos).getFloatTemperature(topPos))) {
