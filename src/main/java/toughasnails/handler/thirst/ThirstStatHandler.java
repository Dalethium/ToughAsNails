			...
			if (target.canBeAttackedWithItem()) {
				if (!target.hitByEntity(player)) {
					...
					if (target instanceof EntityLivingBase) {
						weaponAttackDamage = EnchantmentHelper.getModifierForCreature(player.getHeldItem(EnumHand.MAIN_HAND),
					...
					if (attackDamage > 0.0F || weaponAttackDamage > 0.0F) {
						...
						if (canAttack) {
							// The only part of this method that is new - the rest is recreating the surrounding circumstances in
