...
public class TemperatureStatTableHandler {
	...
	public void onPostRenderOverlay(RenderGameOverlayEvent.Post event) {
		...
		if (event.getType() == ElementType.ALL && debugger.isGuiVisible()) {
			String targetProgress =
					"Target " + TextFormatting.RED + currentTemp + "/" + debugger.targetTemperature + getCappedText(debugger.targetTemperature);
	...
	private static String getCappedText(int targetTemperature) {
		return TextFormatting.BLUE + " " + (targetTemperature < 0 ? "(0)"
				: targetTemperature > TemperatureScale.getScaleTotal() ? "(" + TemperatureScale.getScaleTotal() + ")" : "");
	}
