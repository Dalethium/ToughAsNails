...
public class ModAchievements {
	...
	private static void addAchievements() {
		...
		life_or_death =
				addAchievement("achievement.life_or_death", "life_or_death", 1, 3, new ItemStack(TANItems.lifeblood_crystal), campfire_song);
		...
		that_time_of_year = addAchievement("achievement.that_time_of_year", "that_time_of_year", 3, -1, new ItemStack(TANItems.season_clock),
				thirst_quencher);
		year_one = addAchievement("achievement.year_one", "year_one", 0, -4, new ItemStack(TANItems.tan_icon), that_time_of_year).setSpecial();
	}

	private static Achievement addAchievement(String unlocalizedName, String identifier, int column, int row, ItemStack iconStack,
			Achievement parent) {
