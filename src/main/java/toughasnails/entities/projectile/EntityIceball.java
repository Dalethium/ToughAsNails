...
public class EntityIceball extends Entity implements IProjectile {
	...
	public void onUpdate() {
		if (this.world.isRemote
			...
			for (this.rotationPitch = (float) (MathHelper.atan2((double) f1, this.motionY) * 180.0D / Math.PI) - 90.0F;
			...
			if (this.isInWater()) {
				for (int j = 0; j < 4; ++j) {
					...
					this.world.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX - this.motionX * (double) f3,
							this.posY - this.motionY * (double) f3, this.posZ - this.motionZ * (double) f3, this.motionX, this.motionY,
			...
			ToughAsNails.proxy.spawnParticle(TANParticleTypes.SNOWFLAKE, this.world, this.posX, this.posY + 0.5D, this.posZ, 0.0D, 0.0D, 0.0D,
					new int[0]);
			this.setPosition(this.posX, this.posY, this.posZ);
		} else {
			this.setDead();
